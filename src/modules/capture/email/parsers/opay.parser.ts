...
export class OpayParser implements IEmailParser {
  public readonly parserId = '6ba7b810-9dad-11d1-80b4-00c04fd430c7'
  public readonly bankName = 'OPay'
  public readonly supportedDomains = ['opay-nigeria.com'] as const
  ...
  public async parse(subject: string, bodyHtml: string, bodyText: string): Promise<ParsedTransaction | null> {
    ...
    if (!text.toLowerCase().includes('opay') && !subject.toLowerCase().includes('opay')) {
      return null
    }
    ...
    const merchantName = descMatch && descMatch[1] ? descMatch[1].trim() : 'OPay Transaction'
