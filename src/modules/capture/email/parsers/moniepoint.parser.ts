...
export class MoniepointParser implements IEmailParser {
  public readonly parserId = '6ba7b810-9dad-11d1-80b4-00c04fd430c8'
  public readonly bankName = 'Moniepoint'
  public readonly supportedDomains = ['moniepoint.com'] as const
  ...
  public async parse(subject: string, bodyHtml: string, bodyText: string): Promise<ParsedTransaction | null> {
    ...
    if (!text.toLowerCase().includes('moniepoint') && !subject.toLowerCase().includes('moniepoint')) {
      return null
    }
    ...
    const descMatch = text.match(/(?:Narration|Description|Remarks?|Recipient|Sender|Narrative)\s*:\s*([^;.]+)/i)
    const merchantName = descMatch && descMatch[1] ? descMatch[1].trim() : 'Moniepoint Transaction'
